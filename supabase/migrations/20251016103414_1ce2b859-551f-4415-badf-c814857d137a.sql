-- Добавляем новые колонки для Пирамиды КЭМП и Нутрициологии в таблицу leaderboard
ALTER TABLE public.leaderboard 
ADD COLUMN IF NOT EXISTS kamp_pyramid_points INTEGER DEFAULT 0,
ADD COLUMN IF NOT EXISTS nutrition_points INTEGER DEFAULT 0;

-- Обновляем функцию update_user_leaderboard для отдельного подсчета баллов
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(SUM(CASE WHEN session_type = 'ofp' OR activity_type = 'ofp' THEN points_earned ELSE 0 END), 0),
  ...
  INTO v_bjj_points, v_kick_points, v_ofp_points, v_theory_points, v_tactical_points, v_kamp_pyramid_points, v_nutrition_points
  FROM training_sessions 
  WHERE user_id = user_uuid AND verified = true;

  -- Баллы из тактических сессий (добавляются к v_tactical_points)
  ...
  -- Краш-тесты по типам
  SELECT 
    COALESCE(SUM(CASE WHEN test_type = 'bjj' THEN points_earned ELSE 0 END), 0),
    COALESCE(SUM(CASE WHEN test_type = 'kickboxing' THEN points_earned ELSE 0 END), 0),
    COALESCE(SUM(CASE WHEN test_type NOT IN ('bjj', 'kickboxing') THEN points_earned ELSE 0 END), 0)
  INTO v_crash_bjj, v_crash_kick, v_crash_tactics
  FROM crash_tests
  WHERE user_id = user_uuid AND verified = true;
  
  v_bjj_points := v_bjj_points + v_crash_bjj;
  v_kick_points := v_kick_points + v_crash_kick;
  v_tactical_points := v_tactical_points + v_crash_tactics;
  
  -- Баллы из других активностей (hero_races, homework, lectures, ascetic → к tactical_points)
  SELECT 
  ...
  -- Общий подсчет баллов как сумма всех категорий
  v_total_points := v_bjj_points + v_kick_points + v_ofp_points + v_theory_points + v_tactical_points + v_kamp_pyramid_points + v_nutrition_points;
  ...
  -- Обновляем позиции только для видимых участников (не админов)
