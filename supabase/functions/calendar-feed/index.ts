...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Fetch all active schedule events
    const { data: schedules, error } = await supabase
      .from('schedules')
      .select('id, title, description, start_time, end_time, location, activity_type')
      .eq('is_active', true)
    ...
    const icalLines: string[] = [
      ...
      'CALSCALE:GREGORIAN',
      'METHOD:PUBLISH',
      'X-WR-CALNAME:КЭМП - Расписание',
      'X-WR-TIMEZONE:Europe/Moscow',
      'X-WR-CALDESC:Расписание тренировок и мероприятий КЭМП',
    ...
    return new Response(icalContent, {
      headers: {
        ...
        'Content-Disposition': 'inline; filename="kemp-schedule.ics"',
