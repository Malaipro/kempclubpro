-- Исправление расчета total_points: сумма всех категорий
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(SUM(CASE WHEN session_type = 'tactics' OR activity_type = 'tactics' OR session_type = 'kamp_pyramid' OR activity_type = 'kamp_pyramid' THEN points_earned ELSE 0 END), 0)
  INTO v_bjj_points, v_kick_points, v_ofp_points, v_theory_points, v_tactical_points
  FROM training_sessions 
  WHERE user_id = user_uuid AND verified = true;
  ...
  -- ИСПРАВЛЕНО: Общий подсчет баллов как сумма всех категорий
  v_total_points := v_bjj_points + v_kick_points + v_ofp_points + v_theory_points + v_tactical_points;
...
BEGIN
  FOR participant_record IN 
    SELECT user_id FROM profiles WHERE approved = true
  LOOP
    PERFORM update_user_leaderboard(participant_record.user_id);
  END LOOP;
END $$;